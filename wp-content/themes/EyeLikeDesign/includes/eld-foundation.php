 ...
 *
 * @since  EyeLikeDesign 0.3.1
 *
 * @return string
 */
function eyelikedesign_pagination() {
	global $wp_query;
	...
	$paginate_links = paginate_links( array(
		...
		'mid_size' => 5,
		'prev_next' => True,
	    'prev_text' => __( '&laquo;', 'eyelikedesign' ),
	    'next_text' => __( '&raquo;', 'eyelikedesign' ),
		'type' => 'list'
	) );

	// Display the pagination if more than one page is found
	if ( $paginate_links ) {
		echo '<div class="eld-pagination">';
		echo $paginate_links;
 ...
 * @return string
 */
function eyelikedesign_menu_fallback() {
 ...
 * @return array
 */
function eyelikedesign_active_nav_class( $classes, $item ) {
    if ( $item->current == 1 || $item->current_item_ancestor == true ) {
        $classes[] = 'active';
    }
    return $classes;
}
add_filter( 'nav_menu_css_class', 'eyelikedesign_active_nav_class', 10, 2 );
 ...
 */
function eyelikedesign_active_list_pages_class( $input ) {
...
add_filter( 'wp_list_pages', 'eyelikedesign_active_list_pages_class', 10, 2 );

/**
 * class eyelikedesign_walker
 *
...
class REQ_Foundation_Walker extends Walker_Nav_Menu {
	...
	function __construct( $nav_args = '' ) {
		...
		$this->nav_bar = apply_filters( 'eld_nav_args', wp_parse_args( $nav_args, $defaults ) );
	}
 ...
 * @since EyeLikeDesign 0.5.0
 */
function eyelikedesign_side_nav( $nav_args = '' ) {
	...
	$nav_args = apply_filters( 'eld_side_nav_args', wp_parse_args( $nav_args, $defaults ) );
    ...
    if ( $nav_args['show_home'] == true ) {
    	$nav_args['before'] .= '<li><a href="' . get_home_url() . '">' . __( '&larr; Home', 'eyelikedesign' ) . '</a></li><li class="divider"></li>';
    }
 ...
 *
 * @since EyeLikeDesign 1.0.5
 */
function eyelikedesign_filter_post_class( $classes ) {
...
add_filter( 'post_class', 'eyelikedesign_filter_post_class', 20 );